...
async function sendFileFromUrl(from, url, caption, mek, men) {
    ...
    const sendButMessage = async (
     ...
     text1, 
      ...
      client.sendMessage(
        ...
        {quoted: fgclink, contextInfo: { mentionedJid: [sender], externalAdReply: { title: fake, body: fake2, previewType: "PHOTO", thumbnail: fs.readFileSync(thumb1), thumbnailUrl: thumb2, sourceUrl: fakeUrl}
                }}
      );
    };
    ...
    const sendButLocation = async (
      ...
      _mhan = await client.prepareMessage(from, kma, location, {quoted: fgclink, thumbnail: kma})
...
//ANTI BULE
    ...
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await client.setStatus('[ 🔒 ] Self Bot')
    }

    if (!isOwner && banChats === true) return;
 //SCRIPT ORI BY YOGIPW
//JANGAN DI HAPUS KALO GAK ERROR
  ...
  case "verify": case "daftar":
      ...
      if (!isUser) return sendButMessage(from, mess.notRegis, mess.notRegis1, [{ buttonId: `${prefix}verify`, buttonText: {displayText: verifyButton,}, type: 1,},])
        var ucapan_menu = `${ucapanWaktu} *@${sender.split("@")[0]}*
...
└──────────────────
`;
        sendButImage(from, ucapan_menu, menu, thumbb, [
          {
